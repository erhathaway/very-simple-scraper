...
exports.extractProxyTokenCLIArg = (command) => {
  if (command === "scrape") {
    ...
    if (!tokenArg) {
      console.log(
        "Specify a proxy token with: `node run.js --token=MYTOKEN`"
      );
      return;
    }
...
exports.extractToCLIArg = () => {
  ...
  const token = tokenArg.split('=')[1]
  if (!token) {
      console.log('Specify a start after position for the current csv file name with a `to` option: `node run.js --to=500`')
      return -1
  }   
...
exports.extractFromCLIArg = () => {
  ...
  const token = tokenArg.split('=')[1]
  if (!token) {
      console.log('Specify a start after position for the current csv file name with a `from` option: `node run.js --from=500`')
      return 0
  }   
  return parseInt(token)
}